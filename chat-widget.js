...
chatStyles.innerHTML = `
    ...
    .chat-box-wrapper { position: fixed; bottom: 95px; right: 25px; width: 380px; max-width: 90vw; height: 500px; background: var(--chat-bg, #e5ded8); border: 1px solid var(--glass-border); border-radius: 20px; box-shadow: 0 15px 35px rgba(0,0,0,0.2); z-index: 1000; display: none; flex-direction: column; overflow: hidden; transition: all 0.4s cubic-bezier(0.16, 1, 0.3, 1); transform-origin: bottom right; }
    ...
    .chat-header { background: var(--primary-blue); color: white; padding: 12px 20px; display: flex; justify-content: space-between; align-items: center; font-weight: 700; box-shadow: 0 2px 5px rgba(0,0,0,0.1); z-index: 10; }
    ...
    .chat-header select option { background: var(--card-bg); color: var(--text-dark); }
    ...
    .message-content.doctor { background: var(--primary-blue); color: white; border-top-right-radius: 0; }
    .message-content.doctor::before { right: -10px; border-top-color: var(--primary-blue); border-left-color: var(--primary-blue); }
    .message-content.reception { background: var(--primary-green); color: white; border-top-left-radius: 0; }
    .message-content.reception::before { left: -10px; border-top-color: var(--primary-green); border-right-color: var(--primary-green); }
    ...
    .msg-actions button { background: var(--card-bg); color: var(--text-dark); border: 1px solid var(--glass-border); width: 28px; height: 28px; border-radius: 50%; cursor: pointer; display: flex; justify-content: center; align-items: center; font-size: 11px; box-shadow: 0 2px 5px rgba(0,0,0,0.1); transition: 0.2s; }
    .msg-actions button:hover { background: var(--primary-blue); color: white; transform: scale(1.1); border-color: transparent;}
    .msg-actions button.delete-btn:hover { background: var(--danger); color: white; }

    .message-meta { display: flex; align-items: center; gap: 4px; font-size: 11px; margin-top: 4px; opacity: 0.6; padding: 0 5px; color: var(--text-dark); font-weight: 600;}
    ...
    .chat-input-area { padding: 12px 15px; background: var(--card-bg); display: flex; gap: 10px; border-top: 1px solid var(--glass-border); align-items: center; position: relative; z-index: 10; }
    .chat-input-area input { flex: 1; padding: 12px 20px; border: 1px solid var(--glass-border); border-radius: 25px; font-family: 'Cairo'; font-size: 14.5px; outline: none; background: var(--bg-color); color: var(--text-dark); box-shadow: inset 0 2px 5px rgba(0,0,0,0.02); transition: 0.3s; }
    .chat-input-area input:focus { border-color: var(--primary-blue); box-shadow: 0 0 8px rgba(0, 147, 208, 0.2); }
    .send-btn { background: linear-gradient(135deg, var(--primary-blue), var(--primary-green)); color: white; border: none; width: 45px; height: 45px; border-radius: 50%; cursor: pointer; display: flex; justify-content: center; align-items: center; font-size: 16px; transition: 0.3s; box-shadow: 0 4px 10px rgba(0,0,0,0.15); }
...
window.toggleChat = function() {
    ...
    if(isHidden) {
        ...
        window.checkAndMarkSeen(); 
    }
};

window.changeUserRole = function() {
    localStorage.setItem('chat_current_role', document.getElementById('userRole').value);
    window.renderMessages(chatMessagesData); 
};
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    chatRef.on('value', (snapshot) => {
        ...
        if (initialLoadDone && newMessages.length > chatMessagesData.length) {
            ...
            if (lastMsg.role !== currentRole) playNotificationSound();
        }
...
window.checkAndMarkSeen = function() {
...
window.renderMessages = function(messages) {
    ...
    messages.forEach(msg => {
        ...
        let actionsHtml = isMe ? `<div class="msg-actions"><button onclick="window.startEdit('${msg.id}', '${msg.text.replace(/'/g, "\\'")}')" title="تعديل"><i class="fa-solid fa-pen"></i></button><button class="delete-btn" onclick="window.deleteMessage('${msg.id}')" title="حذف"><i class="fa-solid fa-trash"></i></button></div>` : '';
    ...
    if (isAtBottom) window.scrollToBottom();
};
...
window.startEdit = function(id, oldText) {
    ...
    sendBtn.innerHTML = '<i class="fa-solid fa-check"></i>'; sendBtn.classList.add('edit-mode');
    document.getElementById('cancelEditBtn').style.display = 'block';
};

window.cancelEdit = function() {
    editingMsgId = null; document.getElementById('chatInput').value = '';
    const sendBtn = document.getElementById('sendBtn');
    sendBtn.innerHTML = '<i class="fa-solid fa-paper-plane"></i>'; sendBtn.classList.remove('edit-mode');
...
window.scrollToBottom = function() { const container = document.getElementById('chatMessages'); container.scrollTop = container.scrollHeight; };
